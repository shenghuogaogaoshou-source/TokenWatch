...
**金额口径：全站只用人民币。** 三家官网本来返回的就是人民币，直接展示；唯一带美元原值的是
CC Switch 本地记录（`total_cost_usd` 与 `model_pricing` 都是美元计价），服务端按 `usd_cny`
（默认 7.10）折成人民币后再进入统计，界面上会标注「本地记录原为美元，已按 ¥7.10/USD 折算」。
